...
978         CONTINUE
        ...
        subroutine race(in_lm, in_sglm, in_extm, in_sgextm, out_res)
            use mpmodule
            implicit none
            integer nwds, ndigits
            parameter ( ndigits = %(n_digits)d, nwds = int (ndigits / mpdpw + 2) )
...
!           Cast all inputs to arbitrary precision
            do J=0,3
                do I=0,%(n_lm)d-1
                    lm(I,J) = mpreald(in_lm(I,J), nwds)
                enddo
                do I=0,%(n_sglm)d-1
                    sglm(I,J) = mpreald(in_sglm(I,J), nwds)
                enddo
                do I=0,%(n_extm)d-1
                    extm(I,J) = mpreald(in_extm(I,J), nwds)
                enddo
                do I=0,%(n_sgextm)d-1
                    sgextm(I,J) = mpreald(in_sgextm(I,J), nwds)
                enddo
            enddo

            res = mpreal ('0.0d0', nwds)
